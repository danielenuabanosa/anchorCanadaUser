...
export function OrganizationStatusCard({ className = '' }: { className?: string }) {
  return (
    <div className={cn('flex flex-col rounded-[10px] border border-[#EEF2F8] bg-white p-5', className)}>
      ...
      <div className="mt-5 flex flex-1 flex-col gap-4">
        <div className="flex items-center justify-between py-2.5">
          ...
          <span className="inline-flex items-center gap-1 rounded-[6px] bg-[#ECFDF5] px-1.5 py-0.5 text-sm font-medium text-[#15803D]">
            {ORG_STATUS.verification}
        ...
        <div>
          <div className="mb-2 flex items-center justify-between py-2.5">
            ...
            <span className="text-sm font-medium text-[#0F172A]">{ORG_STATUS.profileComplete}% Complete</span>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-[#EEF2F8]">
            <div className="h-full rounded-full bg-[#2F66C8]" style={{ width: `${ORG_STATUS.profileComplete}%` }} />
          </div>
        </div>

        <div className="flex items-center justify-between py-2.5">
          <span className="text-sm text-[#44516A]">Member Since</span>
          <span className="text-sm font-medium text-[#0F172A]">{ORG_STATUS.memberSince}</span>
        </div>
      </div>
