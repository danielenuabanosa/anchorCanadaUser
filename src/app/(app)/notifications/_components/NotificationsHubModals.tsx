...
import { NOTIFICATION_PREFS, type NotificationItem } from './notificationsData';
  ...
  return (
    <ModalOverlay onClose={onClose} mobile={mobile}>
      <div className="overflow-hidden rounded-[20px] border border-[#D9E1EF] bg-white shadow-[0px_6px_16px_rgba(0,0,0,0.08)]">
        ...
        <div className="flex flex-col px-[26px] pb-8 pt-2">
          <span className={cn('flex h-12 w-12 items-center justify-center rounded-[16px]', item.iconBg)}>
            <Icon className={cn('h-6 w-6', item.iconColor)} strokeWidth={1.75} />
          </span>
          <h2 className="mt-5 font-serif text-[28px] leading-tight text-[#0F172A] md:text-[32px]">{item.title}</h2>
          <p className="mt-2 font-sans text-[13px] text-[#8C97AD]">{item.time}</p>
          <p className="mt-4 font-sans text-[15px] leading-relaxed text-[#44516A]">
    ...
    <>
      <div className="rounded-[10px] border border-[#EEF2F8] bg-white p-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Link href="/notifications" className="text-sm font-medium text-[#2F66C8]">
            View All
          </Link>
        </div>
        <ul>
          {recentActivity && recentActivity.length ? (
            recentActivity.map((item) => {
              ...
              const inner = (
                <div className="flex items-center gap-4">
                  <span className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-[20px] p-2.5', meta.iconBg)}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-sm text-[#44516A]">
                      {item.subtitle}
                      {item.subtitle ? <span className="mx-2.5">•</span> : null}
                      {item.time}
                    </p>
                  </div>
                </div>
              ...
              return (
                <li key={item.id} className="py-3.5">
                  {item.link ? (
                    <Link href={item.link} className="block rounded-[8px] hover:bg-[#F8FAFC]">
                      {inner}
                    </Link>
                  ) : (
                    inner
            ...
            <li className="py-6 text-sm text-[#8C97AD]">No recent activity yet.</li>
          )}
        </ul>
      </div>
