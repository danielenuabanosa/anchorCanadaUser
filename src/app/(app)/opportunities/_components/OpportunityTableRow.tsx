...
export function OpportunityTableRow({ row, onDelete }: OpportunityTableRowProps) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 border-b border-[#EEF2F8] py-[14px] last:border-b-0 md:grid-cols-[minmax(220px,1.4fr)_100px_100px_120px_90px_130px_140px_48px] md:items-center md:gap-2.5">
      ...
      <div>
        <p className="text-sm text-[#44516A]">{row.applicationsDisplay ?? row.applications}</p>
        <p className="text-xs text-[#8C97AD]">{row.applicationsDelta}</p>
      </div>
      <p className="text-sm text-[#44516A]">{row.views.toLocaleString()}</p>
      <div>
        <p className="text-sm text-[#44516A]">{row.deadline}</p>
        <p
          className={`text-xs ${row.daysLeft === 'Closed' || row.daysLeft === '-' ? 'text-[#8C97AD]' : 'text-[#15803D]'}`}
        >
          {row.daysLeft}
        </p>
      </div>
      <div className="flex items-center gap-2">
        {row.health !== '-' ? (
          <>
            ...
            <p className="truncate text-sm text-[#44516A]">{row.health}</p>
          </>
        ) : (
